...
import { normalizeKeyword } from "./keywordNormalize.mjs";
...
test("keyword normalizer: casing / separators collapse", () => {
  const forms = ["Twin-linked", "Twin-Linked", "TWIN-LINKED", "Twin Linked"];
  const keys = forms.map(f => JSON.stringify(normalizeKeyword(f)));
  assert.equal(new Set(keys).size, 1);
  assert.deepEqual(normalizeKeyword("Anti-Fly 4+"), { kw: "anti fly", on: 4 });
  assert.deepEqual(normalizeKeyword("Lethal Hits: non-MONSTER/VEHICLE"), { kw: "lethal hits", vs: "non monster/vehicle" });
  assert.deepEqual(normalizeKeyword("Rapid Fire 2"), { kw: "rapid fire", val: "2" });
});
