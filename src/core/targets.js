...
export const TARGETS=[
    ...
    // Meta group is ordered hardest-to-remove (left) to easiest (right), by
    // effective wounds (ewCalc: total wounds adjusted for save/invuln/FNP,
    // AP0 baseline - same durability metric the "EW/pt" column already uses
    // elsewhere in the app). This intentionally ignores Toughness: wounding
    // probability depends on the attacker's own Strength, not a fixed
    // property of the target, so there's no single "hard to wound" number
    // without assuming a specific attacker - durability here means "hard to
    // put wounds through once hit," consistent with the rest of the app.
    // Allarus EW=288, C'tan/Defiler EW=216 (tied), Magnus EW=192, Cust/Poss
    // EW=180 (tied), B'crusher EW=144, Rubric EW=120, Riptide EW=84, Necron
    // EW=80, Keeper/Fiends EW=72 (tied).
